...
impl Default for Settings { fn default() -> Self { Self { auto_clean: true, interval_minutes: 15, hotkey: "ctrl+alt+KeyR".into(), clean_temp: true, trim_background_apps: false, start_with_windows: false, close_to_tray: default_close_to_tray(), dark_mode: false } } }
...
fn close_background_apps() -> u32 {
    // Only user applications belong here. Never terminate driver, audio, RGB, or vendor service processes.
    let names = ["OneDrive.exe", "Teams.exe", "AdobeIPCBroker.exe", "AdobeCollabSync.exe"];
